...
`rest` is when the mechanical object has returned into itself. In other words,
the object’s determinateness went out of itself through `communication`, it
collided with other objects in `action` and `reaction`, and the object has
returned to being itself by expelling this determinateness entirely. Now, this
...
mechanical objects and so not simply indifferent. It is self-subsistent in the
