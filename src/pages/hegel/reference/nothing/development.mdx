...
Similar to [`being`](/hegel/reference/being/development#examination), the
account of `nothing` begins with a fragment. The first word states the category
in question which is then followed up by a qualifier that this category is to be
...
```md
&ndash; In so far as mention can be made here of intuiting and thinking, it
...
[inline comment](/hegel/reference/being/development#examination) about the
development so far.
...
intuiting) is still _thinking_, and that in itself is not empty.
...
[presuppositionless thinking](/hegel/guides/presuppositionless-thinking) is long
broken and the many assumptions here accrue only more philosophical debt.
...
```md
&ndash; Nothing is therefore the same determination or absence of determination,
and thus altogether the same as what pure _being_ is.
```
